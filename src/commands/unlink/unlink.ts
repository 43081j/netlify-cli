...
import { styleText, netlifyCommand } from '../../utils/command-helpers.js'
...
export const unlink = async (_options: OptionValues, command: BaseCommand) => {
  ...
  if (!siteId) {
    log(
      `Folder is not linked to a Netlify project. Run ${styleText('cyanBright', `${netlifyCommand()} link`)} to link it`,
    )
    return exit()
  }
