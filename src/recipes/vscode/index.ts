...
import { NETLIFYDEVLOG, NETLIFYDEVWARN, styleText, logAndThrowError, log } from '../../utils/command-helpers.js'
...
const getPrompt = ({ fileExists, path }) => {
  const formattedPath = styleText('underline', path)
...
export const run = async ({ config, repositoryRoot }) => {
  ...
  try {
    ...
    log(
      `${NETLIFYDEVWARN} Unable to install Deno VS Code extension. To install it manually, visit ${styleText('blue', 'https://ntl.fyi/deno-vscode')}.`,
    )
  }
