...
const RED_BACKGROUND = styleText('red', '-background')
const [PRO, BUSINESS, ENTERPRISE] = ['Pro', 'Business', 'Enterprise'].map((plan) => styleText('magenta', plan))
...
`
export const MISSING_AWS_SDK_WARNING = `A function has thrown an error due to a missing dependency: ${styleText('yellow', 'aws-sdk')}.
You should add this module to the project's dependencies, using your package manager of choice:

${styleText('yellow', 'npm install aws-sdk --save')} or ${styleText('yellow', 'yarn add aws-sdk')}
