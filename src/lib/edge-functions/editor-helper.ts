...
export const promptEditorHelper = async ({ NETLIFYDEVLOG, config, log, repositoryRoot, state }) => {
  ...
  if (!confirm) {
    log(
      `${NETLIFYDEVLOG} You can start this configuration manually by running ${styleText(['magenta', 'bold'], 'netlify recipes vscode')}.`,
    )
